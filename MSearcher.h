...
ExactSearcher::ExactSearcher(int _k, int _n, vector<pair<int,int>> &_vp, vector<int> &_KDC, int _UB):KDC(_KDC){
		...
		for (int i = 0; i < n; i++){
			...
			// fill(MuEx[i].begin(), MuEx[i].end(), false);
		}	
		...
		for(int i=0;i<n;++i){
			...
			neiInC[i]=neiInG[i];//初始时neiInC和neiInG相等
			PC[i]=i;
			PC_rid[i]=i;
		}
		pstart[n]=idx;
...
bool ExactSearcher::CtoPwithPrune(int u, int level){
	...
	int nonadj_inP=P_end-1-neiInP[u];//和P的负边度数
	int nonadj_inC=(C_end-P_end)-neiInC[u];//和C的负边度数
...
void ExactSearcher::DSetBranch(int level, int P_UB){
	...
	CtoP(u,level);
	// CtoPwithPrune(u,level);
	// removeV_end=removed_v.size();
	// restoreVE(level, old_removeV_end2);
	if(false){
...
void ExactSearcher::removeNonCN(int level){//去除CX中不是P的公共邻居的点
	// printf("enter remove\n");
	for (int i = P_end; i < C_end; ){
		int u = PC[i];
...
int ExactSearcher::Changbd4newBB(){
	...
	int maxCol=0;colorSz[0]=0;
	vector<vector<int>> nonNeiBucket(kCur+1);
	vector<int> weiBucket(kCur+n);
	for (int i = P_end; i < CX_end; i++){
		int u= PC[i];
	...
	//对w进行桶排序, 计算上界
	for (int w = 0; w < k-MEInP+n; w++)
	{
		if(kCur-w*weiBucket[w]>=0) CMax+=weiBucket[w], kCur-=w*weiBucket[w];
		else {
			CMax+=kCur/w;
			break;
		}
...
int ExactSearcher::dpcolorbd4DSetBB(){
	// printf("enter dpcolorbound\n");
	int CMax=0, kCur=k-MEInP;
	// int maxCol=0;colorSz[0]=0;colVec[0].clear();
...
bool ExactSearcher::existSatV(){//注意v_0
	// bool flag=false;
...
bool ExactSearcher::isDSet(){//注意v_0
	for (int i = 0; i < P_end; i++){//scan the vertex in P
		int u = PC[i];
...
public:
	...
	//vertex removing
	int *remove_level;
	int *restore_set;//0代表P, 1代表C, 2代表CX, 3代表CX 
	vector<int> removed_v;
	vector<int> peeled_v; // the vertex moved from C to CX
	vector<int> include_v;
	queue<int> Qv;
   ...
   ExactSearcher2(int _k, int _n, vector<pair<int,int>> &_vp, vector<int> &_KDC, int _UB)
   :KDC(_KDC)
   {
		n=_n;
		m=_vp.size();
   ...
   ~ExactSearcher2(){
	...
	bool isInCX(int u){
		return PC_rid[u] >= C_end && PC_rid[u] < CX_end;
	}
	bool isInX(int u){
		return PC_rid[u] >= CX_end && PC_rid[u] < n;
	}
	int checkEdges(){
		int edgeSum=0;
		for (int i = P_end; i < CX_end; i++){
	...
	void C_coloring(){
		vector<int> C_;
		for (int i = P_end; i < C_end; i++) C_.push_back(PC[i]);
		sort(C_.begin(),C_.end(),[&](int c1,int c2){
			if(neiInG[c1]>neiInG[c2]) return true;
			return false;
		});
		int col,u,maxCol=0;
		for (auto v:C_){
			u=v,col=0;
			while(colorUseMtx[u*n+col]==treeIdx) col++;
			if(col>maxCol) maxCol=max(maxCol,col);
			for (int j = pstart[u]; j < pstart[u+1]; j++){
				int nei=edges[j];
				colorUseMtx[nei*n+col]=treeIdx;
			}
	...
	void C2P(int id){
		...
		MEInP+=nonadj_inP;
		MEInC-=((C_end-P_end)-(neiInG[u]-neiInP[u]));
		for (int i = pstart[u]; i < pstart[u+1]; i++) 
			neiInP[edges[i]]++;
	}
	...
	void P2C(int push_cnt){
		while (push_cnt--)
		{
			MEInC+=(C_end-P_end)-(neiInG[PC[P_end-1]]-neiInP[PC[P_end-1]]);
			P_end--;
	...
	void X2C(int pop_cnt){
		while (pop_cnt--)
		{
			MEInC+=(C_end-P_end)-(neiInG[PC[C_end]]-neiInP[PC[C_end]]);
			C_end++;
...
RET:
	...
	int candiChoose(){
		...
		for (int i = P_end; i < C_end; i++)
		{
			int maxNeiInP=neiInP[i], maxId=i;
			for (int j = i+1; j < C_end; j++){
				if(neiInP[PC[j]]>maxNeiInP){
					maxNeiInP=neiInP[PC[j]], maxId = j;
				}
			}
			if(maxId!=i) swapID(i,maxId);
			if(kCur-(P_end-maxNeiInP)>=0) kCur-=(P_end-maxNeiInP),CMax++;
			else break;
		}
		return CMax+P_end;
	}
	...
	void maxClique(){
		...
		if(CBK0ProMax() <=LB){colorBndPrune++;goto REC;}
		tree_cnt++;
		C2P(pid); maxClique(); P2X(); maxClique(); X2C(1);
	REC:
	...
	double dense=(double)MuExNum/((C_end-P_end)*(C_end-P_end-1)/2+0.001);
	if(C_end-P_end>=1)denSum+=dense,denNum++;
	// denSum+=dense; denNum++;
	// printf("del density: %.2f%%\n",dense*100);
	MaxMuExNum=max(MaxMuExNum,MuExNum);
	MuExSum+=MuExNum;
	...
	void branch(){
		...
		if (P_end > LB) store(P_end);
		if (MEInG<=k) {store(C_end); goto REC;}
		if(k-MEInP==0){ maxClique(); goto REC;}

		edgeDel();
// #ifdef _CLIQUEC_
// 		if(k-MEInP==0){ maxCliqueC(); goto REC;}
// #endif
		treeIdx++;
		for(int id=P_end;id<C_end;id++) (neiInP[PC[pid]]>neiInP[PC[id]])&&(pid=id),neis+=(neiInP[PC[id]]==P_end);
#ifdef DPBOUND
		// if(dpcolorbd()<=LB) {colorBndPrune++; goto REC;}
		if(ChangBound()<=LB) {colorBndPrune++; goto REC;}
#endif
#ifdef INITBOUND
		if(bound(neis) <=LB){boundPrune++;goto REC;}
#endif
#ifdef _CANDIBOUND_
		if(candiBucket2()<=LB){boundPrune++;goto REC;}
#endif
		tree_cnt++;
	...
	void multibranch(){
		...
		tree_cnt++;
		while (P_end<C_end){
			for(int id=P_end;id<C_end;id++) (neiInP[PC[pid]]>neiInP[PC[id]])&&(pid=id),neis+=(neiInP[PC[id]]==P_end);//找到degInP最小的点的位置
			// pid=P_end;
			C2P(pid);
			neis=0, pid=P_end;
			multibranch();
...
REC:
	...
	void search(){
		...
		szSum+=this->n;
		subNum++;
		// printf("the size of subgraph is: %d\n", this->n);
		// CtoP(u);
